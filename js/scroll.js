...
const hamburgerInner = document.querySelectorAll(".hamburger-inner");
...
window.addEventListener("scroll", () => {
  ...
  if (scrolled > 500) {
    proyect_titule.classList.replace(
      "gallery__titule--opacity",
      "gallery__titule--move"
    );
    proyect_text.classList.replace(
      "gallery__text--opacity",
      "gallery__text--move"
    );
  ...
  if (scrolled > 1300) {
    $block__container.classList.replace(
      "block__container--opacity",
      "block__container--move"
    );
  }
  if (scrolled > 1910) {
    for (let i = 0; i < hamburgerInner.length; i++) {
      const element = hamburgerInner[i];
